...
contract Minter is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function setMetadata(
        ...
        attributes[tokenId] = Attr(_name, _title, _imageUri, _traitsKeys, _traitsValues, editMetadataQuantity);
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        for (uint256 i = 0; i < attributes[tokenId].traitsKeys.length; i++) {
            traits[i] = string(
                abi.encodePacked(
                    '{"trait_type": "',
                    attributes[tokenId].traitsKeys[i],
                    '", "value": "',
                    attributes[tokenId].traitsValues[i],
                    '"}'
                )
            );
        }
